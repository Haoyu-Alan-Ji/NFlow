...
def sample_posterior_latents(model, R: int = 2000) -> Dict[str, torch.Tensor]:
    """
    Draw posterior samples from the learned flow posterior and decode them.

    Expected model interface:
        model.q0.rsample(R)
        model.posterior_flow(z0, return_logdet=True)
        model.generative_model.decode(eps)

    Returns CPU tensors for:
        s, u, t, beta
    """
    model.eval()
...
def hard_support_from_draws(
    draws: Dict[str, torch.Tensor],
    support_threshold: float = 0.5,
) -> Dict[str, Any]:
    """
    Convert relaxed posterior draws into a hard support estimate.

    The relaxed rule is:
        gamma_j = 1{u_j > t_j}
        beta_hard_j = s_j * gamma_j

    support_score / vote_rate:
        P(gamma_j = 1 | q)
    """
...
def posterior_predictions_from_hard_samples(
    X,
    beta_hard_samples,
    family: str = "gaussian",
) -> torch.Tensor:
    """
    Generate posterior predictive draws from hard beta samples.
...
def support_metrics(
    ...
    """
    Backward-compatible wrapper for benchmark core.
    """
    return selection_metrics_from_support(
        support_idx=selected_support,
        beta_true=beta_true,
...
def gaussian_predictive_metrics(
    y_true,
    y_pred,
    sigma2: Optional[float],
) -> Dict[str, float]:
    """
    Gaussian predictive metrics based on point predictions.

    Used by mean-field baselines, where y_pred is usually X @ E[beta].
    """
...
def posterior_predictive_metrics_from_hard_samples(
    X,
    y,
    beta_hard_samples,
    ...
    """
    Predictive metrics based on posterior hard beta samples.

    This is the old framework.py predictive_metrics function, renamed
    to make clear that it uses posterior draws rather than point estimates.
    """
    X = _to_torch_float_cpu(X)
...
# Backward-compatible alias for existing framework.py code.
def predictive_metrics(
    ...
    return posterior_predictive_metrics_from_hard_samples(
        X=X,
        y=y,
...
def prob_abs_gt_eps(mu, sd, eps: float):
    """
    Compute P(|beta_j| > eps) for beta_j ~ N(mu_j, sd_j^2).

    Used as quasi-PIP for continuous shrinkage methods such as ARD
    and Bayesian lasso.
    """
